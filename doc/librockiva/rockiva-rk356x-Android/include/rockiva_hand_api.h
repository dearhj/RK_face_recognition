...
typedef enum {
    ...
    ROCKIVA_GESTURE_TYPE_TWO = 2,           /* 2 */
    ROCKIVA_GESTURE_TYPE_THREE = 3,         /* 3 */
    ROCKIVA_GESTURE_TYPE_FOUR = 4,          /* 4 */
    ROCKIVA_GESTURE_TYPE_FIVE = 5,          /* 5 */
    ROCKIVA_GESTURE_TYPE_FIST = 6,          /* 0/拳头 */
    ROCKIVA_GESTURE_TYPE_OK = 7,            /* OK */
    ROCKIVA_GESTURE_TYPE_MAX = 8,
} RockIvaGestureType;
...
typedef struct
{
    ...
    uint8_t minWidth;       /* 最小手大小[0-10000] */
    uint8_t minDetScore;    /* 最小手检测分[0-100] */
    uint8_t sensitivity;    /* 手势识别灵敏度[1,100]，默认0为50灵敏度。灵敏度越高，越容易确定稳定手势，准确度越低。 */
    uint16_t stableTime;    /* 手势识别稳定时间，单位毫秒。默认0为3000ms。超过稳定时间后确定一个稳定手势。 */
